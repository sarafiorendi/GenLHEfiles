...
#           echo "set cluster_temp_path `echo $RUNHOME`" >> mgconfigscript 
  ...
  if [ "$queue" == "condor" ]; then
    echo "set cluster_type condor" >> ./$MGBASEDIRORIG/input/mg5_configuration.txt
...
if grep -q -e "\$DEFAULT_PDF_SETS" -e "\$DEFAULT_PDF_MEMBERS" $CARDSDIR/${name}_run_card.dat; then
    ...
    if [ "$isnlo" -gt "0" ]; then
        if [ $is5FlavorScheme -eq 1 ]; then
            # 5F PDF
                  sed "s/\$DEFAULT_PDF_SETS/263000,306000,322500,322700,322900,323100,323300,323500,323700,323900,305800,13000,13065,13069,13100,13163,13167,13200,25200,25300,25000,42780,90200,91200,90400,91400,61100,61130,61200,61230,13400,82200,292200,292600,315000,315200,262000/" $CARDSDIR/${name}_run_card.dat > ./Cards/run_card.dat
            sed -i "s/\$DEFAULT_PDF_MEMBERS/True,True,False,False,False,False,False,False,False,False,True,True,False,False,True,False,False,False,True,True,False,True,True,True,True,True,True,True,True,True,True,True,True,False,False,False,False/" ./Cards/run_card.dat 
        else
            # 4F PDF
        ...
        # 5F PDF
        if [ $is5FlavorScheme -eq 1 ]; then
            sed "s/\$DEFAULT_PDF_SETS/263000/g" $CARDSDIR/${name}_run_card.dat > ./Cards/run_card.dat
        else
...
#   cp ${LOGFILE} ./gridpack_generation_263000.log
#   DEBUGTARBALL=${name}_debug_tarball.tar.gz
#   tar -czps --ignore-failed-read -f ${DEBUGTARBALL} processtmp gridpack_generation_263000.log
...
#   set -e
  ...
  cp $PRODHOME/runcmsgrid_LO_263000.sh ./runcmsgrid.sh
  sed -i s/SCRAM_ARCH_VERSION_REPLACE/${scram_arch}/g runcmsgrid.sh
  sed -i s/CMSSW_VERSION_REPLACE/${cmssw_version}/g runcmsgrid.sh
...
for i in ${LOGFILE_NAME}*.log; do 
    cp $i ${i/$LOGFILE_NAME/gridpack_generation_263000}; 
done
...
XZ_OPT="$XZ_OPT" tar -cJpsf ${PRODHOME}/${name}_${scram_arch}_${cmssw_version}_tarball.tar.xz mgbasedir process runcmsgrid.sh gridpack_generation_263000*.log InputCards $EXTRA_TAR_ARGS
