...
#   cp ${LOGFILE} ./gridpack_generation_260.log
#   DEBUGTARBALL=${name}_debug_tarball.tar.gz
#   tar -czps --ignore-failed-read -f ${DEBUGTARBALL} processtmp gridpack_generation_260.log
...
for i in ${LOGFILE_NAME}*.log; do 
    cp $i ${i/$LOGFILE_NAME/gridpack_generation_260}; 
done
...
XZ_OPT="$XZ_OPT" tar -cJpsf ${PRODHOME}/${name}_${scram_arch}_${cmssw_version}_tarball.tar.xz mgbasedir process runcmsgrid.sh gridpack_generation_260*.log InputCards $EXTRA_TAR_ARGS
