...
# Check if TMPDIR its already set
if [ "x$TMPDIR" != "x" ]; then
        # If already set, check we can write on it
        if ! [ -w $TMPDIR ]; then
                echo "WARN \$TMPDIR($TMPDIR) is not a writable directory setting \$TMPDIR = \$PWD"
                export TMPDIR=$PWD
        fi
# If TMPDIR is not set or the path is not writable set to default $PWD
else
        export TMPDIR=$PWD
fi
